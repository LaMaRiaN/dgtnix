...
#   default $1 = $PACKAGE_VERSION
#   default $2 = <none>
#
# DESCRIPTION
#
#   This macro is the successor of AC_SET_RELEASEINFO_VERSIONINFO but
#   it can be used in parallel because it uses all different variables.
#
#   check the $VERSION number and cut the two last digit-sequences off
#   which will form a -version-info in a @VERSION_INFO@ ac_subst while
#   the rest is going to the -release name in a @RELEASE_INFO@
#   ac_subst.
#
...
#   e.g. RELEASE_INFO="$RELEASE_INFO-debug" for a debug version of your
#   lib. The $VERSION_INFO however should not be touched.
#
...
#
#   and executables will get link-resolve-infos for libmy-2.so.4 -
#   therefore the patch-level is ignored during ldso linking, and ldso
#   will use the one with the highest patchlevel. Using just "-release
#   $(VERSION)" during libtool-linking would not do that - omitting the
#   -version-info will libtool install libmy.so libmy.la libmy.a
#   libmy-2.4.18.so and executables would get hardlinked with the
...
#   $MAJOR_VERSION.$MINOR_VERSION.$MICRO_VERSION but does not ac_subst
#   for the plain AX_SET_VERSION_INFO macro. Use instead one of the
#   numbered macros AX_SET_VERSION_INFO1 (use first number for release
#   part) or that AX_SET_VERSION_INFO2 (use the first two numbers for
#   release part).
#
#   You may add sublevel parts like "1.4.2-ac5" where the sublevel is
#   just killed from these version/release substvars. That allows to
#   grab the version off a .spec file like with AX_SPEC_PACKAGE_VERSION
#   where the $VERSION is used to name a tarball or distpack like
#   mylib-2.2.9pre4
#
...
#   TEST_MAJOR_VERSION=2... and of course $TEST_RELEASE_INFO etc. (for
#   the moment, it needs to be a literal prefix *sigh*)
#
# LAST MODIFICATION
#
#   2006-10-13
